...
import { DemographicFilters, FilterOptions, DateRangeParams } from '@/lib/admin-api';
...
export function FinancialClinicFilters({
  ...
  const removeFilterValue = (filterKey: Exclude<keyof DemographicFilters, 'unique_users_only'>, value: string) => {
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <Select
            value={dateParams.dateRange || (showCustomDatePicker ? 'custom' : '30d')}
              ...
              <div>
                ...
                <DatePickerComponent
                  ...
                  disabled={(date) => {
                    ...
                    return date > today || date < minDate || (startDate && date < startDate)
                  }}
                />
              </div>
              <Button
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Age Group Filter */}
            <div>
              <Label className="text-xs font-semibold mb-2 block">Age Group</Label>
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, ageGroups: values.length > 0 ? values : undefined })}
            ...
            <div>
              ...
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, genders: values.length > 0 ? values : undefined })}
            ...
            {/* Nationality Filter */}
            <div>
              <Label className="text-xs font-semibold mb-2 block">Nationality</Label>
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, nationalities: values.length > 0 ? values : undefined })}
            ...
            {/* Emirate Filter */}
            <div>
              <Label className="text-xs font-semibold mb-2 block">Emirate</Label>
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, emirates: values.length > 0 ? values : undefined })}
            ...
            <div>
              <Label className="text-xs font-semibold mb-2 block">Employment</Label>
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, employmentStatuses: values.length > 0 ? values : undefined })}
            ...
            {/* Income Range Filter */}
            <div>
              <Label className="text-xs font-semibold mb-2 block">Income Range</Label>
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, incomeRanges: values.length > 0 ? values : undefined })}
            ...
            <div>
              ...
              <MultiSelect
                ...
                onChange={(values) => onFiltersChange({ ...filters, children: values.length > 0 ? values : undefined })}
            ...
            <div>
              <Label className="text-xs font-semibold mb-2 block">Company</Label>
              <MultiSelect
                options={Array.from(
                  new Map(
                ...
                onChange={(values) => onFiltersChange({ ...filters, companies: values.length > 0 ? values : undefined })}
          ...
          <div className="mt-6 pt-4 border-t">
            ...
            <p className="text-xs text-muted-foreground mt-1 ml-6">
              Filter by unique email address, showing only the most recent submission for each user
            </p>
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {filters.unique_users_only && (
                  ...
                  >
                    Unique Users Only
                    <button
                      onClick={() => onFiltersChange({ ...filters, unique_users_only: undefined })}
                ...
                {Object.entries(filters).map(([filterKey, values]) =>
                  filterKey !== 'unique_users_only' && Array.isArray(values) && values.map((value: string) => (
                    <Badge
                      key={`${filterKey}-${value}`}
                      variant="secondary"
                      className="flex items-center gap-1"
                    >
                      {value}
                      <button
                        onClick={() => removeFilterValue(filterKey as Exclude<keyof DemographicFilters, 'unique_users_only'>, value)}
                        className="ml-1 hover:text-destructive"
                      >
                        <X className="w-3 h-3" />
                      </button>
