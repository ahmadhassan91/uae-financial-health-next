...
export function ScheduleEmailModal({ open, onOpenChange, currentFilters }: ScheduleEmailModalProps) {
    const [emails, setEmails] = useState<string[]>([]);
    const [emailInput, setEmailInput] = useState('');
    const [subject, setSubject] = useState(`Consultation Leads Export - ${new Date().toLocaleDateString()}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await apiClient.request('/consultations/admin/schedule-email', {
                method: 'POST',
                body: JSON.stringify({
                    recipient_emails: emails,
                    subject: subject,
                    scheduled_datetime: scheduledDate.toISOString(),
                    status_filter: currentFilters.statusFilter !== 'all' ? currentFilters.statusFilter : null,
                    source_filter: currentFilters.sourceFilter !== 'all' ? currentFilters.sourceFilter : null,
                })
            ...
            if (response.success) {
                ...
                setEmails([]);
                setEmailInput('');
                setSubject(`Consultation Leads Export - ${new Date().toLocaleDateString()}`);
            ...
            toast.error(error?.message || 'An error occurred while scheduling the email');
        } finally {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px]">
                ...
                <div className="space-y-4 py-4">
                    ...
                    {/* Active Filters Info */}
                    {(currentFilters.statusFilter !== 'all' || currentFilters.sourceFilter !== 'all') && (
                        <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
                            <p className="text-sm font-medium text-blue-900 mb-1">Active Filters:</p>
                            <div className="text-sm text-blue-700 space-y-1">
                                {currentFilters.statusFilter !== 'all' && (
                                    <div>• Status: <span className="font-medium capitalize">{currentFilters.statusFilter}</span></div>
                                )}
                                {currentFilters.sourceFilter !== 'all' && (
                                    <div>• Source: <span className="font-medium capitalize">{currentFilters.sourceFilter.replace('_', ' ')}</span></div>
                                )}
                            </div>
                        </div>
                    )}
