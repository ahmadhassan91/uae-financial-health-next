...
export function FinancialClinicSurvey({
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      try {
        ...
        const fetchedQuestions = await fetchFinancialClinicQuestions(
          profile.children
        );
  ...
  const answeredQuestionsCount = questions.filter(
    (q) => getResponse(q.id) !== undefined
  ).length;
  const totalSteps = questions.length;
  const progress =
    totalSteps > 0 ? (answeredQuestionsCount / totalSteps) * 100 : 0;
          ...
          <div className="w-full max-w-[1380px] px-1 sm:px-2">
            ...
            <div className="flex flex-col gap-2 md:gap-3 w-full">
              {currentCategoryQuestions.map((question) => {
                const questionText =
                  language === "ar" ? question.text_ar : question.text_en;
                    ...
                    <CardContent className="flex flex-col sm:flex-row items-start gap-3 md:gap-4 lg:gap-6 p-3 sm:p-4 md:p-5 lg:p-8">
                      ...
                      <div className="flex flex-col items-start gap-2 flex-1 w-full min-w-0">
                        ...
                        <RadioGroup
                          ...
                          onValueChange={(value) =>
                            onResponse(question.id, parseInt(value))
                          }
