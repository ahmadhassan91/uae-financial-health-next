...
import { Building2, Plus, Link, Eye, EyeOff, Trash2, Copy, QrCode, Calendar, Users, Download, Info, Edit } from 'lucide-react';
...
export function CompanyManagement({ onBack }: CompanyManagementProps) {
  const { companies, loading, createCompany, deleteCompany, generateLink } = useCompanyManagement();
  ...
  const [availableVariationSets, setAvailableVariationSets] = useState<Array<{
    id: number;
    name: string;
    description: string;
    set_type: string;
  ...
  const fetchAvailableVariationSets = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/variation-sets?is_active=true&page_size=100`, {
        headers: {
  ...
  const handleDownloadQR = () => {
    if (qrCodeInstance.current) {
      ...
      qrCodeInstance.current.download({
        name: `${companyName.replace(/\s+/g, '-').toLowerCase()}-financial-clinic-qr`,
        extension: 'png'
      });
  ...
  const handleUpdateCompany = async () => {
    try {
      const response = await apiClient.updateCompany(editingCompany.id, newCompany);
  ...
  const handleToggleActive = async (company: any) => {
    try {
      ...
      toast.success(newStatus ? 'Company activated!' : 'Company deactivated - survey link disabled');
      // Refresh companies list
      window.location.reload();
    } catch (error: any) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage corporate clients and their financial health assessment programs
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewCompany(prev => ({ ...prev, company_email: e.target.value }))}
              ...
              <div className="space-y-3 border-t pt-4">
                ...
                <p className="text-sm text-muted-foreground">
                  Assign a pre-configured set of 15 question variations to this company.
                </p>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    setNewCompany(prev => ({ 
                      ...prev, 
                      variation_set_id: value === 'default' ? null : parseInt(value)
                    }));
                  }}
                >
                  ...
                  <SelectContent>
                    <SelectItem value="default">Use Default Questions (Recommended)</SelectItem>
                ...
                <p className="text-xs text-muted-foreground">
                  💡 Tip: Variation sets contain 15 pre-configured question variations. Create sets in System Management → Question Variation Sets.
                </p>
              </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button 
                  ...
                  disabled={!newCompany.company_name || !newCompany.company_email || !newCompany.contact_person}
                >
                  Create Company
                </Button>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {companies.reduce((sum, company) => sum + company.total_assessments, 0)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {companies.length > 0 
                ? Math.round(companies.reduce((sum, c) => sum + (c.average_score || 0), 0) / companies.length)
                : 0
              }
            </div>
          </CardContent>
          ...
          {companies.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-4">
                Start by adding your first company to the financial health program
              </p>
                  ...
                  <TableRow key={company.id}>
                    <TableCell className="font-medium">
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">{company.company_email}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                    ...
                    <TableCell>
                      {company.question_variation_mapping && 
                       Object.keys(company.question_variation_mapping).length > 0 ? (
                        <Badge variant="secondary" className="gap-1">
                          <span>Custom ({Object.keys(company.question_variation_mapping).length})</span>
                        </Badge>
                      ) : (
                    ...
                    <TableCell>
                      {company.average_score ? Math.round(company.average_score) : 'N/A'}
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          title={company.is_active ? "Deactivate Survey Link" : "Activate Survey Link"}
                        >
      ...
      <Dialog open={showLinkDialog} onOpenChange={setShowLinkDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Share this Financial Clinic assessment link with {selectedCompany?.company_name} employees.
              All completed assessments will be automatically tracked to your company dashboard.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="text-sm text-muted-foreground text-center">
              This QR code will direct users to:<br />
              <span className="font-mono text-xs break-all">{generatedLink}</span>
            </div>
