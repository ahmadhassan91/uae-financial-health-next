...
import {
  Users, TrendUp, Clock, Pulse, Download,
  FileText, Funnel
...
export function AdminDashboard({ scoreHistory, onExportData, onBack }: AdminDashboardProps) {
  ...
  const analytics = useMemo(() => {
    ...
    const timeAndDemographicFiltered = filteredData.filter(score =>
    ...
    const nationalityBreakdown = timeAndDemographicFiltered.reduce((acc, score) => {
    ...
    const employmentBreakdown = timeAndDemographicFiltered.reduce((acc, score) => {
    ...
    // Score analytics
    const averageScore = timeAndDemographicFiltered.length > 0
      ? Math.round(timeAndDemographicFiltered.reduce((sum, score) => sum + score.totalScore, 0) / timeAndDemographicFiltered.length)
      : 0;

    const pillarAverages = Object.entries(PILLAR_DEFINITIONS).map(([pillar, def]) => ({
      pillar: def.name,
      average: timeAndDemographicFiltered.length > 0
        ? Number((timeAndDemographicFiltered.reduce((sum, score) => {
          const pillarScore = score.pillarScores?.find(p => p.pillar === pillar);
          return sum + (pillarScore && isValidPillarScore(pillarScore) ? pillarScore.score : 0);
        }, 0) / timeAndDemographicFiltered.length).toFixed(1))
        : 0,
      fullMark: 5
    }));
    ...
    return {
      ...
      demographics: {
        ageGroups: Object.entries(ageGroups).map(([age, count]) => ({
          age, count, percentage: Math.round((count / totalCompletions) * 100)
        })),
        nationality: Object.entries(nationalityBreakdown).map(([nat, count]) => ({
          nationality: nat, count, percentage: Math.round((count / totalCompletions) * 100)
        })),
        income: Object.entries(incomeBreakdown).map(([income, count]) => ({
          income, count, percentage: Math.round((count / totalCompletions) * 100)
        })),
        employment: Object.entries(employmentBreakdown).map(([emp, count]) => ({
          employment: emp, count, percentage: Math.round((count / totalCompletions) * 100)
        }))
      },
      scores: {
        pillarAverages,
  ...
  const COLORS = ['#1B365D', '#D4AF37', '#22C55E', '#F59E0B', '#EF4444', '#8B5CF6', '#06B6D4'];
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-4 md:mt-0">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => onExportData('csv', hasActiveFilters)} className="flex items-center gap-2">
                <Download className="w-4 h-4" />
                Export CSV {hasActiveFilters && '(Filtered)'}
              </Button>
              <Button variant="outline" onClick={() => onExportData('excel', hasActiveFilters)} className="flex items-center gap-2">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{analytics.overview.totalCompletions}</div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{Math.floor(analytics.overview.averageCompletionTime / 60)}m {analytics.overview.averageCompletionTime % 60}s</div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={analytics.overview.averageScore >= 65 ? 'default' : analytics.overview.averageScore >= 50 ? 'secondary' : 'destructive'}>
                      {analytics.overview.averageScore >= 65 ? 'Excellent' : analytics.overview.averageScore >= 50 ? 'Good' : 'Needs Improvement'}
                    </Badge>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Financial health score ranges</CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={[
                      { range: '15-34', count: Math.floor(analytics.overview.totalCompletions * 0.15), label: 'Needs Improvement' },
                      { range: '35-49', count: Math.floor(analytics.overview.totalCompletions * 0.25), label: 'Fair' },
                      { range: '50-64', count: Math.floor(analytics.overview.totalCompletions * 0.35), label: 'Good' },
                      { range: '65-80', count: Math.floor(analytics.overview.totalCompletions * 0.25), label: 'Excellent' }
                    ]}>
          ...
          <TabsContent value="leads" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Manage consultation requests from the Financial Clinic</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-center py-8">
                  <p className="text-muted-foreground">Leads management feature coming soon...</p>
                  <p className="text-sm text-muted-foreground mt-2">Integration with consultation API in progress</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Survey participants by age group</CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        ...
                        cy="50%"
                        labelLine={false}
                        label={({ employment, percentage }) => `${employment}: ${percentage}%`}
                        outerRadius={80}
                        fill="#8884d8"
                        dataKey="count"
                      >
                        {analytics.demographics.employment.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                        ))}
                      </Pie>
                      <Tooltip />
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Average scores and percentages for each pillar</CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    {analytics.scores.pillarAverages.map((pillar, index) => (
                      <div key={pillar.pillar} className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant="outline">{pillar.average}/5.0</Badge>
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Daily survey completions over time (filtered data)</CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <AreaChart data={analytics.scores.trendData}>
                      ...
                      <YAxis />
                      <Tooltip />
                      <Area type="monotone" dataKey="completions" stroke="#1B365D" fill="#1B365D" fillOpacity={0.2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Financial health score evolution (filtered data)</CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={analytics.scores.trendData}>
                      ...
                      <YAxis domain={[15, 80]} />
                      <Tooltip />
                      <Line type="monotone" dataKey="averageScore" stroke="#D4AF37" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
          ...
          <TabsContent value="data" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Complete list of all financial health assessments submitted by users
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <TableBody>
                      {analytics.filteredData.length === 0 ? (
                        ...
                        analytics.filteredData
                          ...
                          .map((record) => {
                            const interpretation = record.totalScore >= 60 ? 'Excellent' :
                              record.totalScore >= 45 ? 'Good' :
                                record.totalScore >= 30 ? 'Needs Improvement' : 'At Risk';
                            const interpretationColor = interpretation === 'Excellent' ? 'bg-green-100 text-green-800' :
                              interpretation === 'Good' ? 'bg-blue-100 text-blue-800' :
                                interpretation === 'Needs Improvement' ? 'bg-yellow-100 text-yellow-800' :
                                  'bg-red-100 text-red-800';

                            return (
                              <TableRow key={record.id}>
                                <TableCell className="font-medium">{record.profile.name}</TableCell>
                                <TableCell>{new Date(record.createdAt).toLocaleDateString()}</TableCell>
                                <TableCell>{record.profile.age}</TableCell>
                                <TableCell>{record.profile.nationality}</TableCell>
                                <TableCell>{record.profile.incomeRange}</TableCell>
                                <TableCell className="font-semibold">
                                  {record.totalScore}/{record.maxPossibleScore}
                                </TableCell>
                                <TableCell>
                                  <Badge variant="secondary" className={interpretationColor}>
                                    {interpretation}
                                  </Badge>
                                </TableCell>
                                <TableCell>
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      alert(`Detailed view for ${record.profile.name} - Score: ${record.totalScore}/${record.maxPossibleScore}`);
                                    }}
                                  >
                                    View Details
                                  </Button>
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                  <Card>
                    <CardContent className="p-4">
                      <div className="text-2xl font-bold">{analytics.filteredData.length}</div>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <div className="text-2xl font-bold">
                        {analytics.filteredData.length > 0 ? Math.round(analytics.filteredData.reduce((sum, r) => sum + r.totalScore, 0) / analytics.filteredData.length) : 0}
                      </div>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <div className="text-2xl font-bold">
                        {analytics.filteredData.filter(r => r.totalScore >= 60).length}
                      </div>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <div className="text-2xl font-bold">
                        {analytics.filteredData.filter(r => r.totalScore < 30).length}
                      </div>
