...
export function ScoreHistory({
  ...
  const histogramData =
    ...
    Array.isArray(latestScore.pillarScores) &&
    latestScore.pillarScores.length > 0
      ? latestScore.pillarScores
          .filter((pillarScore) => isValidPillarScore(pillarScore))
          .map((pillarScore) => {
            const displayData = getPillarDisplayData(
              pillarScore,
              PILLAR_DEFINITIONS[pillarScore.pillar]?.name
            );
            return {
      ...
      : // Fallback to subScores for backward compatibility
        (latestScore.subScores &&
        Array.isArray(latestScore.subScores) &&
        latestScore.subScores.length > 0
          ? latestScore.subScores
          : []
        )
          .map((subScore) => ({
                ...
                <div className="text-center">
                  ...
                  >
                    {scoreDiff > 0 ? (
                      ...
                      <ChartLineDown className="w-6 h-6" />
                    ) : null}
                    {scoreDiff > 0 ? "+" : ""}
                    {scoreDiff}
                  </div>
            ...
            <CardHeader>
              ...
              <CardDescription>
                <span style={{ color: "#737373" }}>
                  Your financial health score progression over times
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {chartData && chartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData}>
                      ...
                      <YAxis domain={[15, 80]} tick={{ fontSize: 12 }} />
                      <Tooltip
          ...
          <CardContent>
            <div className="space-y-4">
              {scoreHistory.map((calc, index) => {
                ...
                const diff = prevCalc
                  ? (calc.totalScore || 0) - (prevCalc.totalScore || 0)
                  : 0;

                return (
                  <div
                    key={calc.id || `calc-${index}`}
                    className="flex items-center justify-between p-4 border rounded-lg"
                  >
                    <div className="flex items-center gap-4">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-[#bd912e]">
                          {calc.totalScore || 0}
                        </div>
                      ...
                      {diff !== 0 && (
                        ...
                        >
                          {diff > 0 ? (
                            ...
                            <ChartLineDown className="w-4 h-4" />
                          )}
                          {diff > 0 ? "+" : ""}
                          {diff}
                        </div>
                    ...
                    <div className="text-right">
                      <div className="text-sm font-medium">
                        {calc.advice && Array.isArray(calc.advice)
                          ? calc.advice.length
                          : 0}{" "}
                        recommendations
                      </div>
