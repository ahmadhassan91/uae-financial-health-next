...
const LocalizationContext = createContext<
  EnhancedLocalizationContextType | undefined
>(undefined);
...
export function LocalizationProvider({
  ...
  useEffect(() => {
    ...
    if (
      language === "ar" &&
      !document.querySelector('link[href*="Noto+Sans+Arabic"]')
    ) {
  ...
  const loadTranslations = async (lang: SupportedLanguage) => {
    ...
    try {
      ...
      // Merge with defaults and simple translations, API translations take precedence
      const baseTranslations =
        lang === "ar" ? simpleTranslations.ar : simpleTranslations.en;
      ...
      if (Object.keys(cacheRef.current).length >= MAX_CACHE_SIZE) {
        ...
        const oldestKey = Object.keys(cacheRef.current).sort(
          (a, b) =>
      ...
      console.log(
        ...
        } translations for ${lang} from API`
      );
    } catch (error) {
  ...
  const t = (key: string, params?: Record<string, any>): string => {
    try {
      // Priority order: simple static translations > API translations > defaults
      // Check simple static translations first (these include Arabic)
      const languageTranslations =
        simpleTranslations?.[language] || simpleTranslations?.en || {};
      let translation = (languageTranslations as Record<string, any>)[key];

      // Fallback to API translations if not found in simple translations
      if (!translation) {
  ...
  const refreshContent = useCallback(
    async (contentType?: ContentType): Promise<void> => {
      if (contentType) {
  ...
  const preloadContent = useCallback(
    async (contentTypes: ContentType[]): Promise<void> => {
  ...
  const getContentByType = useCallback(
    (type: ContentType): LocalizedContent[] => {
      return allContent.filter(
        (item) => item.content_type === type && item.language === language
      );
    },
    [allContent, language]
  );
  ...
  const getTranslationCoverage = useCallback(() => {
    const enContent = allContent.filter(
      (item) => item.language === "en" && item.is_active
    );
    const arContent = allContent.filter(
      (item) => item.language === "ar" && item.is_active
    );
  ...
  const contentStats: ContentStats = {
    ...
    englishItems: allContent.filter(
      (item) => item.language === "en" && item.is_active
    ).length,
    arabicItems: allContent.filter(
      (item) => item.language === "ar" && item.is_active
    ).length,
