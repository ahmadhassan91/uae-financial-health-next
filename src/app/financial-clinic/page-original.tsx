...
export default function FinancialClinicPage({ restoredSession }: FinancialClinicPageProps) {
  ...
  const [companyTracking, setCompanyTracking] = useState<{active: boolean; companyName?: string} | null>(null);
  ...
  useEffect(() => {
    if (restoredSession && restoredSession.profile) {
      console.log('🔄 Restoring profile from session:', restoredSession.profile);
      ...
      localStorage.setItem('financialClinicProfile', JSON.stringify(restoredSession.profile));
      localStorage.setItem('restoredAnswers', JSON.stringify(restoredSession.responses || {}));
      localStorage.setItem('restoredStep', restoredSession.current_step.toString());
      
      // Navigate to survey with company parameter if present
      const surveyUrl = restoredSession.company_url
        ? `/financial-clinic/survey?company=${encodeURIComponent(restoredSession.company_url)}`
  ...
  const handleInputChange = (field: keyof FinancialClinicProfile, value: string) => {
    setProfile(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleStartSurvey = () => {
    ...
    if (!profile.date_of_birth.trim()) {
      toast.error(language === 'ar' ? 'يرجى إدخال تاريخ الميلاد' : 'Please enter your date of birth');
      return;
    }
  ...
  if (showConsent) {
    return <ConsentModal onConsent={handleConsentGranted} onDecline={handleConsentDeclined} />;
  }
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <div className="text-center">
                <div className="font-semibold text-[#437749]">
                  {language === 'ar' ? '🏢 التقييم المقدم من شركة' : '🏢 Company Assessment'}
                </div>
        ...
        <div className="flex flex-col items-center gap-4 md:gap-6 w-full max-w-[850px]">
            ...
            <div className="flex-1">
              ...
              <Select value={profile.emirate} onValueChange={handleEmirateChange}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Ras Al Khaimah / Fujairah / UAQ / Outside UAE">Ras Al Khaimah / Fujairah / UAQ / Outside UAE</SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 w-full">
            ...
            <div className="flex-1">
              ...
              <SelectContent>
                <SelectItem value="Employed">{language === 'ar' ? 'موظف' : 'Employed'}</SelectItem>
                <SelectItem value="Self-Employed">{language === 'ar' ? 'أعمال حرة' : 'Self-Employed'}</SelectItem>
                <SelectItem value="Unemployed">{language === 'ar' ? 'عاطل عن العمل' : 'Unemployed'}</SelectItem>
              </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex-1">
              ...
              <Select value={profile.income_range} onValueChange={handleIncomeChange}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="10,000 to 20,000">10,000 to 20,000</SelectItem>
                  <SelectItem value="20,000 to 30,000">20,000 to 30,000</SelectItem>
                  <SelectItem value="30,000 to 40,000">30,000 to 40,000</SelectItem>
                  <SelectItem value="40,000 to 50,000">40,000 to 50,000</SelectItem>
                  <SelectItem value="50,000 to 100,000">50,000 to 100,000</SelectItem>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 w-full">
            ...
            <div className="flex-1">
              ...
              <Input
                placeholder={language === 'ar' ? 'أدخل رقم جوالك' : 'Enter your mobile number'}
                type="tel"
