...
import { ArrowLeft, CheckCircle, EnvelopeSimple, Info } from '@phosphor-icons/react';
...
function FinancialClinicLoginContent() {
  ...
  const { loading, error, requestOTP, verifyOTP, resendOTP, resetState } = useOTPAuth(language);
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              {step === 'email' ? (
                <>
                  <EnvelopeSimple className="w-5 h-5 text-blue-600" weight="fill" />
                ...
                <div className="text-center">
                  <Link href={`/financial-clinic?lang=${language}`}>
                    <Button variant="link" className="text-sm">
                      ...
                      {language === 'ar' ? 'العودة إلى العيادة المالية' : 'Back to Financial Clinic'}
                    </Button>
                  </Link>
                </div>
              </form>
