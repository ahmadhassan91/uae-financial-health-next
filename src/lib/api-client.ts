...
class ApiClient {
  ...
  private createEnhancedError(error: any, status?: number, url?: string): ApiError {
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      ...
      detail = 'Cross-origin request blocked. This may be a temporary server issue.';
      code = 'CORS_ERROR';
      retryable = true;
    } else if (status === 408) {
  ...
  private async requestWithRetry<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const enhancedError = this.createEnhancedError(errorData, response.status, url);
        ...
        if (enhancedError.retryable && attempt < this.retryConfig.maxRetries) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(`Request failed (attempt ${attempt + 1}/${this.retryConfig.maxRetries + 1}), retrying in ${delay}ms:`, enhancedError.detail);
          }
      ...
      if (error.name === 'AbortError') {
        ...
        if (attempt < this.retryConfig.maxRetries) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(`Request timeout (attempt ${attempt + 1}/${this.retryConfig.maxRetries + 1}), retrying in ${delay}ms`);
          }
      ...
      if (enhancedError.retryable && attempt < this.retryConfig.maxRetries) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`Network error (attempt ${attempt + 1}/${this.retryConfig.maxRetries + 1}), retrying in ${delay}ms:`, enhancedError.detail);
        }
  ...
  async register(userData: {
    ...
    username: string;
    password: string;
  }): Promise<{ id: number; email: string; username: string; is_active: boolean; is_admin: boolean; created_at: string }> {
  ...
  async updateIncompleteSurvey(sessionId: string, surveyData: {
    current_step?: number;
    responses?: Record<string, any>;
    email?: string;
    phone_number?: string;
  ...
  async generateCompanyLink(companyId: number, config?: {
    prefix?: string;
    expiry_days?: number;
  ...
  async getQuestionVariationsSimple(): Promise<{
    variations_by_question: Record<string, Array<{
      id: number;
      name: string;
      language: string;
      preview: string;
  ...
  async getIncompleteSurveys(skip: number = 0, limit: number = 50, abandonedOnly: boolean = false): Promise<any[]> {
    return this.request(`/surveys/incomplete/admin/list?skip=${skip}&limit=${limit}&abandoned_only=${abandonedOnly}`);
  }
  ...
  async exportIncompleteSurveys(
    ...
    const response = await fetch(`${this.baseUrl}/surveys/incomplete/admin/export?${params}`, {
      method: 'GET',
      headers: {
    ...
    if (contentDisposition) {
      ...
      filename = `incomplete_surveys_${abandonedOnly ? 'abandoned' : 'all'}_${timestamp}.csv`;
    }
  ...
  async getLocalizedContent(
    ...
    return this.request(`/api/localization/content${queryString ? `?${queryString}` : ''}`);
  }
  ...
  async getQuestionsByLanguage(
    ...
    const queryString = params.toString();
    return this.request(`/api/localization/questions/${language}${queryString ? `?${queryString}` : ''}`);
  }
  ...
  async deleteLocalizedContent(contentId: number): Promise<{ message: string }> {
    return this.request(`/api/localization/content/${contentId}`, {
      method: 'DELETE',
    });
  }
  ...
  isProduction(): boolean {
    return this.baseUrl.includes('herokuapp.com') || 
  ...
  async getFinancialClinicHistory(skip: number = 0, limit: number = 50): Promise<any[]> {
    return this.request(`/financial-clinic/history?skip=${skip}&limit=${limit}`);
  }
}
